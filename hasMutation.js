...
function anyAdjacentRepeatedNeighbors(array, [i, j], gene) {
    ...
    let northEastNeighs = [[i - 1, j + 1], [i - 2, j + 2], [i - 3, j + 3]];
    let eastNeighs = [[i, j + 1], [i, j + 2], [i, j + 3]];
    let southEastNeighs = [[i + 1, j + 1], [i + 2, j + 2], [i + 3, j + 3]];
    let southNeighs = [[i + 1, j], [i + 2, j], [i + 3, j]];
